...
export const PUT = async (req: any) => {
    ...
    try {
        ...
        const existingReaction = await reactionsCollection.findOne({ hash: sha256Hash, type: 1 });

        let update = {};

        if (existingReaction) {
            update = { $inc: { meaning_useful_count: -1 } };
        ...
        return NextResponse.json({
            ...
            success: true,
            status: 200,
            useful_count: data.meaning_useful_count
        });
